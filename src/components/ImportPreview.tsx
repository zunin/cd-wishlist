...
export const ImportPreview: FC<ImportPreviewProps> = ({ onBack, onComplete }) => {
  ...
  const [selectedForImport, setSelectedForImport] = useState<Set<string>>(
    () => new Set(items.filter((i) => i.status === "matched" && i.selectedMatch).map((i) => i.id))
  );
  ...
  const multipleCount = items.filter(
    (i) => i.status === "multiple"
  ).length;
  ...
  const handleImport = async () => {
    ...
    const itemsToImport = items.filter(
      (i) => selectedForImport.has(i.id) && i.selectedMatch?.releaseGroupId
    );

    for (const item of itemsToImport) {
      if (item.selectedMatch?.releaseGroupId) {
        dispatch(
          addItem({ id: item.selectedMatch.releaseGroupId })
        );
      }
    }
    ...
    return (
      <div className="import-preview">
        ...
        <div className="matching-progress">
          ...
          />
          <p>
            Matching {progress?.current ?? 0} of {progress?.total ?? 0} albums...
          </p>
        </div>
      </div>
    );
    ...
    <div className="import-preview">
            ...
            <tr>
              <th className="col-select">
                <input
                  type="checkbox"
                  checked={
                    matchedCount > 0 && selectedForImport.size === matchedCount
                  }
                  onChange={(e) =>
                    e.target.checked
            ...
            {items.map((item) => (
              <tr
                ...
                className={`status-${item.status} ${
                  selectedForImport.has(item.id) ? "selected" : ""
                }`}
              >
                ...
                <td className="col-match">
                  {item.selectedMatch ? (
                    <span className="match-info">
                      {item.selectedMatch.artist} — {item.selectedMatch.albumTitle}
                      {item.selectedMatch.type && (
                        <span className="match-type">({item.selectedMatch.type})</span>
                      )}
                    </span>
      ...
      {(unmatchedCount > 0 || multipleCount > 0) && (
        <p className="pending-notice">
          Unmatched items will be saved to your Pending Queue for later.
        </p>
      )}
    </div>
  );
