...
export const ImportColumnMapper: FC<ImportColumnMapperProps> = ({ onBack }) => {
  ...
  const validation = useMemo(() => {
    if (!columnMapping) return { valid: false, error: "No mapping configured" };
  ...
  const handleMappingChange = (columnIndex: number, value: string) => {
    dispatch(updateColumnMapping({ columnIndex, mapping: value as ColumnMapping }));
  };
  ...
  const getMappingCount = (type: ColumnMapping): number => {
    ...
    return columnMapping.mappings.filter((m: ColumnMapping) => m === type).length;
  };
  ...
  return (
    <div className="import-column-mapper">
      ...
      <div className="column-mapping-config">
        <div className="mapping-option">
          <label>
            <input
              type="checkbox"
              checked={columnMapping.hasHeaderRow}
              onChange={(e) => dispatch(updateHasHeaderRow(e.target.checked))}
            />
            First row is header
          </label>
        ...
        <div className="mapping-option">
          ...
          <select
            ...
            onChange={(e) => dispatch(updateDelimiter(e.target.value))}
          >
            {AVAILABLE_DELIMITERS.map((delim) => (
              <option key={delim.value} value={delim.value}>
                {delim.label}
                {delim.value === rawData.detectedDelimiter ? ' (detected)' : ''}
              </option>
            ))}
          </select>
        </div>
      ...
      <div className="mapping-summary">
        <span className={`mapping-count ${getMappingCount('artist') > 0 ? 'valid' : 'invalid'}`}>
          Artist columns: {getMappingCount('artist')}
        </span>
        <span className={`mapping-count ${getMappingCount('album') > 0 ? 'valid' : 'invalid'}`}>
              ...
              {Array.from({ length: columnCount }, (_, i) => (
                <th key={i} className="col-mapping">
                  <div className="column-header">
                    <span className="column-label">Column {i + 1}</span>
                    <select
                      value={columnMapping.mappings[i] || 'skip'}
            ...
            {headerRow && (
              <tr className="header-row">
                ...
                {headerRow.map((cell: string, i: number) => (
                  <td key={i} className="header-cell">{cell}</td>
                ))}
              </tr>
            )}
            {dataRows.map((row: string[], rowIndex: number) => (
              <tr key={rowIndex}>
                <td className="row-label">Row {rowIndex + (columnMapping.hasHeaderRow ? 2 : 1)}</td>
      ...
      {rawData.rows.length > 6 && (
        <p className="preview-more">
          Showing {dataRows.length} of {columnMapping.hasHeaderRow ? rawData.rows.length - 1 : rawData.rows.length} rows
        </p>
      )}
