...
function createProvider(settings: SyncSettings): WebrtcProvider {
    ...
    provider.on(
        ...
        ({
            ...
            if (
                (lastPeerCount === 0 && currentCount === 1) ||
                (lastPeerCount === 1 && currentCount === 0)
            ) {
                oscillationCount++;
            }
...
function bindRespectingExistingData(doc: Doc, store: Store, sliceName: string) {
    ...
    if (
        !hasYjsData &&
        orphanedIds &&
        typeof (orphanedIds as { toJSON?: () => unknown }).toJSON === "function"
    ) {
        transact(doc, () => {
            rootMap.set(sliceName, {
                ids: (orphanedIds as { toJSON: () => unknown }).toJSON(),
            });
            rootMap.delete("ids");
        });
    ...
    const handleYjsChange = () => {
        ...
        if (
            yjsSlice &&
            typeof (yjsSlice as { toJSON?: () => unknown }).toJSON ===
                "function"
        ) {
            store.dispatch({
                type: SET_STATE_FROM_YJS_ACTION,
                payload: (yjsSlice as { toJSON: () => unknown }).toJSON(),
            });
        }
        patchingStore = false;
    };
